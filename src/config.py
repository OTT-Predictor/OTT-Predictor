...
RAW_DATA_FILENAME = "updated2_movie.csv" # 원본 데이터 파일 이름 (우리가 직접 준비해야 할 파일)
...
COMPANY_OHE_FILENAME = "company_onehot_encoder.joblib"
COMPANY_OHE_PATH = os.path.join(MODEL_DIR, COMPANY_OHE_FILENAME)
...
DEEP_HIDDEN_DIMS = [512, 256] # Deep 모델 내부의 중간 계산층(은닉층) 크기 설정
                              # [256, 128]은 첫 번째 중간층은 256개, 두 번째는 128개로 한다는 의미
DROPOUT_RATE = 0.10573 # 모델 학습 시 과도한 암기를 막기 위한 장치(드롭아웃)의 비율

# --- 학습 하이퍼파라미터 (모델 학습 방법을 조절하는 값들) ---
LEARNING_RATE = 0.00124 # 학습률: 모델이 정답을 향해 얼마나 큰 걸음으로 나아갈지 정하는 값 (0.0001)
BATCH_SIZE = 64      # 배치 크기: 한 번에 학습할 데이터 묶음의 크기 (16개씩 묶어서 학습)
...
COMPANY_OHE_PREFIX = 'company_' # 제작사 원-핫 인코딩 접두사
